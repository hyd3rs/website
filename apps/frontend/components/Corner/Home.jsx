...
export const Header = () => (
    <>
        <h1 className="text-6xl font-semibold text-left text-brand-gradient">my <span className="text-purple-600 font-bold">corner*</span></h1>
...
export const Post = ({ slug, title, author, children, integral, timestamp, tags }) => {
    ...
    return (
        <Link href={`/corner/p/${slug || "#"}`}>
            <Card className="group ease-in-out duration-300 rounded-2xl border-2 text-black dark:text-white border-slate-800 hover:bg-slate-800 dark:border-purple-900 dark:hover:bg-purple-900 hover:text-white bg-transparent font-medium">
                <CardHeader>
                    <CardTitle>
                        <div className="flex flex-row items-center gap-x-2">
                            {
                                integral && (
                                    <span className="uppercase font-medium text-base tracking-widest text-cyan-500 w-fit">INTEGRAL</span>
                                )
                            }
                            <span>
                    ...
                    <CardDescription className="group-hover:text-slate-300 dark:group-hover:text-slate-400">
                        by <span className="text-black group-hover:text-white dark:text-white font-bold">{author || "No author provided"}</span>
                        {
                            (tags && typeof tags === "object") && (
                                <span>
                                    {" "} | {" "}
                                    <span className="uppercase font-medium tracking-widest text-purple-800 dark:text-purple-500 group-hover:dark:text-white">{tags?.join(", ")}</span>
                                </span>
                            )
                        }
                    </CardDescription>
                ...
                <CardFooter>
                    <p>{integral ? "Updated" : "Published"} <b>{processedTimestamp}</b></p>
                </CardFooter>
            </Card>
        </Link>
    )
